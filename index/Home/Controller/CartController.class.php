...
class CartController extends ComController{
    ...
    public function cartCount(){
    	$cart = M('cart');
    	$good=M('goods');
    	$del= $cart->field('cart_id,gid')->where(['uid'=>$_SESSION['sc']['id'],'pay'=>3])->select();
		foreach ($del as $key => $value) {
			$goods=$good->field('id')->where(['id'=>$value['gid']])->find();
			if(!$goods){
				$cart->where(['cart_id'=>$value['cart_id'],'uid'=>$_SESSION['sc']['id']])->delete();
			}
		}
    	$cartCount=$cart->field("cart_id")->where(['uid'=>$_SESSION['sc']['id'],'pay'=>3])->count();
    	if($cartCount == 0){
    		$infor=0;
    	}else{
    		$infor=$cartCount;
	...
	public function cart(){
		$info = M('cart')
	            ...
	            ->where(['c.uid'=>$_SESSION['sc']['id'],'c.pay'=>3])
	...
	public function docart(){
	    $shopcart_m = M('cart');
	    $gid =I("post.id","");
	    $kcun=M('goods')->field('kucun')->where(['id'=>$gid])->find();
	    if($kcun['kucun'] > 0){
	    	$cartlist = $shopcart_m->field("buynum,pay")->where(['gid'=>$gid,'uid'=>$_SESSION['sc']['id']])->find();
		    if($cartlist){//如果购物车中存在商品id 购买数量累加
		    	$shoplist['buynum'] = $cartlist['buynum'] + 1;
		    	if($cartlist['pay'] == 2){//已付款
		    		$data['buynum'] = 0;
		    		$data['pay'] = 3;
		    		$shopcart_m->where(['gid'=>$gid,'uid'=>$_SESSION['sc']['id']])->save($data);
		    	}
		    	if(false !== $shopcart_m->where(['gid'=>$gid,'uid'=>$_SESSION['sc']['id']])->save($shoplist)){
