///<jscompress sourcefile="common_ajaxfunction_main.js" />
/* 
 ...
 * 返回值主体：goods
 */
function ajaxPublishResultSaleDetail(gid,pid, sid, publishResidueTimes,callbackFun){
	$.ajax({
		type: "post",
		timeout:timeout,
		dataType: "json",
		...
		url:'/qz/goods/detailSalePublishAjax.do',
		success: function(result) {
			//调用绑定页面数据方法
			if (verification(result)) { //验证result 是否有效
				callbackFun(gid,pid,sid, publishResidueTimes,result);//调用回调函数 传输json结果
			}
		},
		error: function(r) {
			handlingException(r);
...
function ajaxGoodsSaleDetailInfo(gid, callbackFun){
	$.ajax({
		url:'/qz/goods/selectGoodsByGid.do?t='+ Math.random(),
		type:'post',
		dataType:"json",
		async: false,
		data:{
 ...
 */
	 ...
	 * 
	 */
	function ajaxGoodsSaleDetailPublish(sid,callbackFun){
		$.ajax({
			url:'/qz/goods/detailSalePublishAjax.do?t='+ Math.random(),
			type:'post',
			dataType:"json",
			async: false,
			data:{
	 ...
	 * 返回值 json云购码
	 */
	function ajaxShowYgCodesSale(mid,sid,callbackFun){
		$.ajax({
			url:'/qz/goods/selectBuyCodes.do?t='+ Math.random(),
			type:'post',
			dataType:"json",
			async: false,
			data:{
	 ...
	 * @param callbackFun 回调函数
	 */
	function ajaxGetCategoryOneQz(callbackFun){
		$.ajax({
			url:'/qz/goods/categoryIndex.do?t='+ Math.random(),
			type:'post',
			dataType:"json",
			async: false,
			success : function(data){
	 ...
	 * @param callbackFun 回调函数
	 */
	function ajaxGetAllBrand(param,callbackFun, index){
		$.ajax({
	 ...
	 * @param callbackFun 回调函数
	 */
	function ajaxGetAllBrandQz(param,callbackFun){
		$.ajax({
			url:'/qz/goods/getAllBrand.do?t='+ Math.random(),
			type:'post',
			dataType:"json",
			async: false,
			data:param,
...
///<jscompress sourcefile="common_ajaxfunction_other.js" />
/*
 * Description : 所有商品接口 请求函数
 * Author : gaoxiaopeng@ddtkj.com
 * Time : 2015年 9月21日
 ...
 * 
 * */
function ajaxGetPast(parameter,callbackFun){
	$.ajax({
		type: "post",
		timeout:timeout,
		dataType: "json",
		data:parameter,
		cache: true,
		url: "/publish/getPast.do?r=" + Math.random(),
		success: function(result) {
			//调用绑定页面数据方法
			callbackFun(parameter,result);//调用回调函数 传输json结果
		},
		error: function(r) {
			handlingException(r);
		} //错误处理
