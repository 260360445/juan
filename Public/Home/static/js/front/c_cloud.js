// 2015-5-22   start  js代码新增
	/**
	 * 根据类型获取官方QQ群列表
	 * @param type 1直属群  2 地方群
	 * @param callbackFun
	 */
	function ajaxQqGroupList(type,callbackFun){
		$.ajax({
			url:'/cms/selectQqGroupList.do?t='+ Math.random(),
			type:'post',
			dataType:"json",
			async: false,
			data:{
...
// $(".w_common_question_box
// .c_common_question").eq(index).show().siblings().hide();
// })
...
// }
// })
	ajaxQqGroupList(1,bindZSQqGroup);
	//ajaxQqGroupList(2,bindDFQqGroup);
})
// 2015-5-22 end js代码新增
function bindZSQqGroup(result){
	$(result.data).each(function(index,group){
		groupHtmllist='';
		if(group.isOver==1){
		groupHtmllist+='<li>';
		}else{
			groupHtmllist+='<li class="c_nogroup_name">';
		}
		groupHtmllist+='<div class="c_group_left">';
		groupHtmllist+='<img src="../static/img/front/cloud_global/cloud_icon.png" alt=""/>';
		groupHtmllist+='</div>';
		groupHtmllist+='<div class="c_group_right">';
		groupHtmllist+='<span>'+group.title+'</span>';
		groupHtmllist+='<b>'+group.account+'</b>';
		groupHtmllist+='<em></em>';
		groupHtmllist+='</div>';
		if(group.isOver==1){
			groupHtmllist+='<i>该群已满员</i>';
			groupHtmllist+='</li>';
			groupHtmlDatil='';
		}else{
			groupHtmllist+='<i>点击图片可扫描二维码添加</i>';
			groupHtmllist+='</li>';
			groupHtmlDatil='';
			groupHtmlDatil+='<div class="c_qq_detail">';
			groupHtmlDatil+='<div class="c_group_img"><img src="'+imagePath+group.qrCode+'" alt=""/></div>';
			groupHtmlDatil+='<div class="c_group_descrip">';
			groupHtmlDatil+='<p>扫一扫</p>';
			groupHtmlDatil+='<p>'+group.title+'</p>';
			if(typeof(group.url)!="undefined"){
				groupHtmlDatil+='<a href="'+group.url+'">'+group.account+'</a>';
			}else{
				groupHtmlDatil+='<a href="#">'+group.account+'</a>';
			}
			groupHtmlDatil+='</div>';
			groupHtmlDatil+='<div class="c_qq_close"></div>';
			groupHtmlDatil+='</div>';
			$("#zsqDatil").append(groupHtmlDatil);
		}
		$("#zsqlist").append(groupHtmllist);
	})
}
function bindDFQqGroup(result){
	$(result.data).each(function(index,group){
		groupHtmllist='';
		groupHtmllist+='<li>';
		groupHtmllist+='<div class="c_group_left">';
		groupHtmllist+='<img src="../static/img/icon/n_cloud_icon.png" alt=""/>';
		groupHtmllist+='</div>';
		groupHtmllist+='<div class="c_group_right">';
		groupHtmllist+='<span>'+group.title+'</span>';
		groupHtmllist+='<b>'+group.account+'</b>';
		...
		groupHtmlDatil+='<div class="c_qq_close"></div>';
		groupHtmlDatil+='</div>';
		$("#dfqList").append(groupHtmllist);
		$("#dfqDatil").append(groupHtmlDatil);
	})
}
$(function(){
	// 我的购物车 start
	...
	// $(".c_pop_list").width(($(".c_pop_list li").outerWidth())*$(".c_pop_list
	// li").length);
	...
	$(".c_pay_way li").click(function(){
		...
		$("#payTypeMsg").html(title);
		
		var tradeType = $(this).attr("data-val");
		$("#tradeType").val(tradeType);
		
		$(this).addClass("c_pay_this").siblings().removeClass("c_pay_this");
		/*
		 * if(index!=3){ $(".c_pay_bank").hide(); $(".c_select_money").show();
		 * }else{ $(".c_pay_bank").show(); $(".c_select_money").hide(); }
		 */
		switch (index){ 
			case 6 : 
 				$(".c_pay_bank").hide();
...
// break;
	...
	$(".c_select_money li").click(function(){
		...
		$(this).addClass("c_pay_this").siblings().removeClass("c_pay_this");
		 ...
		 $(".c_fan_money_1").hide();
		 if(rechargeSalesShow){
			 $($(".c_fan_money_1")[index]).show();
		 }
		 if(index!=5){
		 $($(".c_fan_money")[index]).show();
		 }
		if(index<5){
	...
	$(window).resize(function(){
		...
		$("#presellWindow").css({left:($(window).width()-$("#presellWindow").width())/2+"px",top:($(window).height()-$("#presellWindow").height())/2+"px"});
	...
	$(".c_nogroup_name").click(function(){
		var index=$(this).index(".c_nogroup_name");
		$(".c_qq_bj").show();
	...
	if(newdaHours>=7&&newdaHours<=10){
		...
		$(".c_person_info").css({background:"url(../../static/img/front/cloud_global/morning.jpg) no-repeat"});// 2015.06.09
																												// 新增
	}else if(newdaHours>10&&newdaHours<=13){
		$(".c_time_period").html("中午好");
		$(".c_person_info").css({background:"url(../../static/img/front/cloud_global/noon.jpg) no-repeat"});// 2015.06.09
																											// 新增
	}else if(newdaHours>13&&newdaHours<=18){
		$(".c_time_period").html("下午好");
		$(".c_person_info").css({background:"url(../../static/img/front/cloud_global/afternoon.jpg) no-repeat"});// 2015.06.09
																													// 新增
	}else{
		$(".c_time_period").html("晚上好");
		$(".c_person_info").css({background:"url(../../static/img/front/cloud_global/evening.jpg) no-repeat"});// 2015.06.09
																												// 新增
	}
	/* 2015.4.30 end */
	/* 2015.5.5 start */
	...
	// $(".c_certificate_content
	// ul").css({width:$(window).width()*$(".c_certificate_content ul
	// li").length});
