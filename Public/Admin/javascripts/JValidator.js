...
//验证数字 num
function Buynum1(obj) {
    reg = /^[-+]?\d+$/;
    if (!reg.test(obj)) {
        return false;
    } else {
        if (parseInt($('#s-buy').val()) < parseInt(obj)) {
            return true;
        }else{
            return false;
        }
...
function JudgeValidate(obj) {
    ...
    $(obj).find("[datacol=yes]").each(function (e) {
        if ($(this).attr("checkexpession") != undefined) {
            switch ($(this).attr("checkexpession")) {
                case "default":
                    ...
                    break;
                case "NotNull":
                    if($(this).val()==$(this).attr("err"))
                    {
                        Validatemsg = $(this).attr("err") + "不能为空！\n";
                        Validateflag = false;
                        ChangeCss($(this), Validatemsg); return false;
                    }
                    else if (isNotNull($(this).val())) {
                        if($(this).attr("value")!=undefined){
                            if($(this).attr("type")=="password"){
                                $(this).attr("type","text");
                            }
                            $(this).val($(this).attr("err"));
                        }                                
                        Validatemsg = $(this).attr("err") + "不能为空！\n";                        
                        Validateflag = false;
                        ChangeCss($(this), Validatemsg); return false;
                ...
                case "CheckBuyNum":
                    if (!Buynum1($(this).val())) {
                        Validatemsg = $(this).attr("err") + "不能小于起始量！\n";
                        Validateflag = false;
                        ChangeCss($(this), Validatemsg); return false;
                    }
                    break;
