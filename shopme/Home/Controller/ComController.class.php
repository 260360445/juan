...
class ComController extends Controller {
	protected function _initialize(){
    	...
    	if(empty($_SESSION['ad']['id'])){
            //跳转到认证网关
            $this->redirect('Login/login');
        }
        /*if($_SESSION['ad']['auth'] != 'all' && CONTROLLER_NAME != 'Index'){
        	$isAuth = CONTROLLER_NAME.'_'.ACTION_NAME;
        	$authArr = explode(',', $_SESSION['ad']['auth']);
        	$res = in_array($isAuth, $authArr);
        	if(!$res){
        		echo "<script>alert('No authority');location.href='".U('Index/index')."';</script>";
        		die;
        	}
        }*/
	}
	...
	static function _mdStr($v1, $v2 = 'wanhuatong') {
	...
	static function _sendCode($mobile) {
		...
		file_get_contents('http://www.wanht.net/sms/jiuwuSendSms.php?mobile='.$mobile.'&code='.$str);
